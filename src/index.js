...
$(document).ready(() => {
  ...
  //         allShadows.push(`${shadowX}px ${0}px ${0}px rgba(0,0,0,0.5)`);
  //       }
  ...
  //         // allShadows.push(`${0}px ${shadowY}px ${shadowY}px rgba(0,0,0,0.15)`);
  //       }
  //       cell.style.boxShadow = allShadows.join(',');
  ...
  $.fn.stickyTable = function (args) {
    ...
    function positionStickyElements(elems, offsetX = 0, offsetY = 0) {
      elems.forEach((cell) => {
        ...
        if (!cell.classList.contains('sticky--is-stuck-y') || cell.classList.contains('sticky--is-stuck')) {
          transforms.push(`translateX(${offsetX}px)`);
          // allShadows.push(`${shadowX}px ${0}px ${0}px rgba(0,0,0,0.15)`);
          cell.style.setProperty(`--shadow-x-offset`, `${shadowX}px`);
        }
        if (!cell.classList.contains('sticky--is-stuck-x') || cell.classList.contains('sticky--is-stuck')) {
          transforms.push(`translateY(${offsetY}px)`);
          // allShadows.push(`${0}px ${shadowY}px ${0}px rgba(255,0,0,0.15)`);
          cell.style.setProperty(`--shadow-y-offset`, `${shadowY}px`);
        }
        // cell.style.boxShadow = allShadows.join(',');
        cell.style.transform = transforms.join(' ');
      });
